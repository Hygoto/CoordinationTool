	...
	while true:
		...
		if file.ends_with(".bplist"):
			...
			if error == OK:
				...
				elif create_pickorder():
					$OptionButton.set_item_tooltip($OptionButton.get_item_count()-1, "invalid or missing pickorder in playlist; using an autogenerated order instead")
					$OptionButton.set_item_icon($OptionButton.get_item_count()-1, warning_icon)
			else:
